...
begin
  ...
  loop
    ...
    set stock = stock + v_item.quantity,
        total_sold = total_sold - v_item.quantity,
        updated_at = now()
...
begin
  ...
  if v_coupon.usage_limit is not null and v_coupon.usage_count >= v_coupon.usage_limit then
    raise exception 'Coupon % has reached maximum usage limit of %', v_coupon.code, v_coupon.usage_limit;
  end if;
...
create or replace function public.confirm_order_update_stock(
  p_order_id uuid
) returns void
language plpgsql
security definer
...
begin
  ...
  loop
    ...
    if not found then
      raise exception 'Insufficient stock for product % during order confirmation', v_item.product_id;
    end if;
  end loop;
end;
$$;